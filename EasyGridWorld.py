import sys
sys.path.append(r"/Users/kevin/Desktop/CS7641/HW4/CODE/burlap.jar")
import java
...
#   Print the grid world problem map
    ...
    for lr in [0.1, 0.5, 0.9]:
        for epsilon in [0.2, 0.8]:
            ...
            for nIter in iterations:
                ...
                '''if (nIter == 1 and lr == 0.1 and epsilon == 0.8):
                    simpleValueFunctionVis(agent, p, initialState, domain, hashingFactory, Qname+str(nIter))
                    break
                if (nIter == 100/2 and lr == 0.1 and epsilon == 0.8):
                    simpleValueFunctionVis(agent, p, initialState, domain, hashingFactory, Qname+str(nIter))
                    break
                if (nIter == 100 and lr == 0.1 and epsilon == 0.8):
                    simpleValueFunctionVis(agent, p, initialState, domain, hashingFactory, Qname+str(nIter))
                    break
                if (nIter == MAX_ITERATIONS and lr == 0.1 and epsilon == 0.8):
                    simpleValueFunctionVis(agent, p, initialState, domain, hashingFactory, Qname+str(nIter))
                    break'''
